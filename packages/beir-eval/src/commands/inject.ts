...
export async function cmdInject(opts: InjectOptions): Promise<void> {
  ...
  if (existing > 0) {
    console.error(`[inject] "${dataset}" already injected (${existing} docs). Run "eject" first.`);
    process.exit(1);
  }

  // Distinct docs that have chunks + embeddings for the requested config
  ...
  // Insert documents one by one and track corpus_id → document_id
  ...
  // Fetch all chunks with embeddings and copy to production chunks table
