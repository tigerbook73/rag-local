...
**Embedding Sidecar**：`services/embedding/` 是独立 Python 进程，api 和 worker 均通过 `EMBEDDING_SERVICE_URL`（默认 `http://localhost:8000`）调 HTTP，`EmbeddingService.init()` 仅设 URL，无模型加载开销。
...
| BGE-M3 Embedding       | Python FastAPI sidecar（`services/embedding/`），PyTorch + CUDA，非 Node.js ONNX               |
| Redis URL 解析         | `parseRedisUrl()`（`packages/core/src/config/redis.ts`），api/worker/health 三处共用           |
| API 路由前缀           | `API_PREFIX = "api/v1"`（`apps/api/src/common/constants.ts`），main.ts 和 gen-spec.ts 共用     |
| Settings DB key 字符串 | `SETTINGS_KEYS` + `SETTINGS_DEFAULTS`（`apps/api/src/modules/settings/settings.constants.ts`） |
...
`lefthook.yml` 的 `no-commit-to-main` 规则**禁止直接向 `main` 提交**，必须先切到功能分支：
...
**commitlint 强制规则（违反会被 hook 拒绝）：**

- **scope 必填** — `feat: add X` ❌，`feat(api): add X` ✅
- subject case 不限制，大写缩写（CRUD、LLM、UI）可直接使用

**常用 scope（对应 monorepo 包/模块）：**
...
| `core`    | `packages/core`                  |
| `db`      | `packages/db`                    |
| `phase-N` | 跨包的阶段性功能（如 `phase-4`） |

类型：`feat`、`fix`、`chore`、`refactor`、`docs`、`test`、`style`、`perf`
