...
src/
  common/
    constants.ts              # API_PREFIX = "api/v1"（main.ts 和 gen-spec.ts 共用）
  ...
  modules/
    ...
    settings/      # GET / PATCH（含 requiresReindex 检测）
      settings.constants.ts   # SETTINGS_KEYS、SETTINGS_DEFAULTS（DB key 字符串集中管理）
  gen-spec.ts      # 生成 OpenAPI JSON 文件的脚本（nest build 后运行）
```

### 关键非默认行为

**启动时初始化（非请求时）：** `MessagesBootstrapService` 实现 `OnApplicationBootstrap`，在应用启动时调用 `EmbeddingService.init()`（同步，仅设置 sidecar URL）和 `LLMService.init()`（从 DB 读 Settings 初始化 OpenAI client）。
